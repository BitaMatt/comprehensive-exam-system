...
- PDF 生成题库：选择 PDF 后自动抽取文本并生成可直接练习的题库；扫描件建议先转成可选中文字 PDF。
...
应用按 OpenAI-compatible `/v1/models` 和 `/v1/chat/completions` 接口调用。当前稳定版优先处理可选中文字 PDF；扫描页会给出提示，建议先用外部 OCR 转成可选中文字 PDF 后再生成。
